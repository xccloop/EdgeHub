...
class RealtimePlot(QWidget):
    def __init__(self):
        ...
        if HAS_PYQTGRAPH:
            ...
            self._pen_colors = ['#80cbc4', '#ff8a80', '#69f0ae', '#ffd740',
                                '#b388ff', '#82b1ff', '#ccff90', '#ff80ab']
            layout.addWidget(self.plot_widget)

        self.param_selector = QComboBox()
        self.param_selector.setFixedHeight(26)
        self.param_selector.setStyleSheet("font-size: 11px;")
        self.param_selector.currentTextChanged.connect(self._on_selection)
    ...
    def add_data_point(self, name: str, value: float):
        if name not in self._data:
            ...
            if HAS_PYQTGRAPH:
                ...
                self._curves[name] = self.plot_widget.plot([], [], pen=pg.mkPen(color=color, width=1.5), name=name)
